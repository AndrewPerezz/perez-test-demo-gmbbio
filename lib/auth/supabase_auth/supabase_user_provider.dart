...
class PerezTestDemoSupabaseUser extends BaseAuthUser {
  PerezTestDemoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> perezTestDemoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PerezTestDemoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
